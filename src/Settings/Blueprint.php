...
final class Blueprint
{
    ...
    public function make(): StatamicBlueprint
    {
        return BlueprintFacade::make()->setContents([
            'sections' => [
                'global' => [
                    'display' => __('magic-actions::magic-actions.settings.global.display'),
    ...
    private function globalFields(): array
    {
        return [
            [
                ...
                'field' => [
                    'type' => 'textarea',
                    'display' => __('magic-actions::magic-actions.settings.global.system_prompt.display'),
                    'instructions' => __('magic-actions::magic-actions.settings.global.system_prompt.instructions'),
                    'rows' => 4,
                ],
            ],
            [
                ...
                'field' => [
                    'type' => 'section',
                    'display' => __('magic-actions::magic-actions.settings.global.defaults.display'),
    ...
    private function typeDefaultFields(): array
    {
        ...
        foreach ($types as $type => $config) {
            ...
            $fields[] = [
                ...
                'field' => [
                    'type' => 'select',
                    'display' => __('magic-actions::magic-actions.settings.global.defaults.type_display', ['type' => ucfirst($type)]),
                    'instructions' => __('magic-actions::magic-actions.settings.global.defaults.type_instructions', ['type' => $type]),
                    'options' => $options,
                    'default' => $config['default'] ?? null,
                    'clearable' => true,
                    'placeholder' => __('magic-actions::magic-actions.settings.global.defaults.placeholder'),
                    'width' => 33,
                ],
            ];
        }
    ...
    private function defaultModelsInstructions(): string
    {
        ...
        $base = __('magic-actions::magic-actions.settings.global.defaults.instructions.base');

        if ($providers === []) {
            return $base.' '.__('magic-actions::magic-actions.settings.global.defaults.instructions.no_providers');
        }

        if ($configured === []) {
            $envKeys = $this->providerEnvKeyList($providers, ', ');

            return $base.' '.__('magic-actions::magic-actions.settings.global.defaults.instructions.no_api_keys', [
                'env_keys' => $envKeys,
            ]);
        }
        ...
        if ($missing !== []) {
            ...
            return $base.' '.__('magic-actions::magic-actions.settings.global.defaults.instructions.unlock_more', [
                'env_keys' => $envKeys,
            ]);
        }
    ...
    private function buildModelOptions(array $models): array
    {
        ...
        foreach ($models as $modelKey) {
            ...
            $options[$modelKey] = __('magic-actions::magic-actions.settings.global.defaults.provider_model_label', [
